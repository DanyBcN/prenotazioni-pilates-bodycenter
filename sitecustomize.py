try:
    ...
    _skip_header_logo_once = False
    _skip_header_caption_once = False
    _nav_css_done = False
    ...
    def columns_patched(spec, *args, **kwargs):
        global _skip_header_logo_once
        if spec == [1, 6] or spec == (1, 6):
            _skip_header_logo_once = True
            return [_NoopColumn(), _NoopColumn()]
        return _original_columns(spec, *args, **kwargs)

    def image_patched(*args, **kwargs):
        global _skip_header_logo_once
        if _skip_header_logo_once and kwargs.get("width") == 130:
            return None
        return _original_image(*args, **kwargs)

    def markdown_patched(body, *args, **kwargs):
        global _skip_header_logo_once, _skip_header_caption_once
        if isinstance(body, str) and "Prenotazioni Pilates Reformer" in body and "<h1" in body:
            ...
            _skip_header_logo_once = False
            _skip_header_caption_once = True
    ...
    def caption_patched(body, *args, **kwargs):
        global _skip_header_caption_once
        if _skip_header_caption_once and isinstance(body, str) and "Gestionale interno Body Center" in body:
            _skip_header_caption_once = False
            return None
